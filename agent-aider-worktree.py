...
def generate_context(worktree_path):
    ...
    with open(context_tmp, "w", encoding="utf-8") as f:
        ...
        if os.path.exists(tests_dir) and os.path.isdir(tests_dir):
            ...
            console.print(
                "[dim]No tests directory found, skipping test execution[/dim]"
            )
...
def run_aider(worktree_path, task, args, model="r1"):
    ...
    # Log file count
    console.print(
        f"[dim]Found {len(all_files)} files in repository ({len(python_files)} Python files)[/dim]"
    )
    ...
    message = (
        f"Improve code quality and fix any issues found in tests or linting. "
    ...
    aider_cmd = (
        f"aider --architect --model {model} --weak-model '{weak_model}' {read_args_str}"
    )
...
def merge_and_push(worktree_path, main_repo_path, branch_name, main_branch, args):
    ...
    try:
        ...
        console.print(
            f"[bold blue]Pulling latest changes from {main_branch}...[/bold blue]"
        )
        ...
        if (
            ...
            if (
                ...
                console.print(
                    "[bold red]Merge conflicts could not be automatically resolved.[/bold red]"
                )
                return False
        ...
        if not tests_pass:
            ...
            if not tests_pass:
                console.print(
                    "[bold red]Tests still failing after attempted fixes.[/bold red]"
                )
                return False

        # Push changes to the branch
        console.print(
            f"[bold blue]Pushing changes to branch {branch_name}...[/bold blue]"
        )
        run_command(f"git push -u origin {branch_name}", cwd=worktree_path)

        # Merge back to main if requested
        console.print(
            f"[bold blue]Merging {branch_name} into {main_branch}...[/bold blue]"
        )
        result = run_command(
        ...
        if result.returncode != 0:
            console.print(
                "[bold red]Failed to merge to main. Please resolve conflicts manually.[/bold red]"
            )
            return False
...
Examples:
  ...
  %(prog)s --inner-loop 5 "Refactor database code"
    ...
    if not os.path.exists(os.path.join(main_repo_path, ".git")):
        console.print(
            f"[bold red]Error:[/bold red] {main_repo_path} is not a git repository"
        )
        sys.exit(1)
    ...
    config_table.add_row(
        "Include All Files", "Yes" if args.all_files else "No (Python only)"
    )
    config_table.add_row(
        "Disable Python Files", "Yes" if args.no_python_files else "No"
    )
    ...
    while iteration <= args.max_iterations:
        ...
        if tests_pass:
            ...
            run_command(
                "git add -A && git commit -m 'Final changes' || true", cwd=worktree_path
            )
